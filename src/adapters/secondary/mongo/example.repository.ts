...
export class MongoExampleRepository implements IExampleRepository {
  ...
  async save(example: Example): Promise<void> {
    ...
    await this.client.db().collection<IExample>(this.COLLECTION).insertOne(example)
  }

  async getAll(): Promise<Example[]> {
    this.logger.info('Retrieving all examples from the database')
    const result = await this.client.db().collection<IExample>(this.COLLECTION).find().toArray()
  ...
  async update(example: Example): Promise<void> {
    ...
    await this.client.db().collection<IExample>(this.COLLECTION).updateOne({ _id: example._id }, { $set: example })
  }

  async getById(_id: string): Promise<Example | undefined> {
    this.logger.info('Retrieving entity by id from the database')
    const result = await this.client.db().collection<IExample>(this.COLLECTION).findOne({ _id })
  ...
  async getByName(name: string): Promise<Example | undefined> {
    ...
    const result = await this.client.db().collection<IExample>(this.COLLECTION).findOne({ name: name })
    return result ? new Example(result) : undefined
  }
}
